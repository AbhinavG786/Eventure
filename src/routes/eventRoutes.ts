...
router.route("/bookmarks").get(authMiddleware.verifyToken,event.getBookmarkedEvents);
...
router.route("/bookmark/:eventId").patch(authMiddleware.verifyToken,event.toggleBookmarkEvent);
router.route("/:id").patch(event.updateEventById);
router.route("/:id").delete(event.deleteEventById);
router.route("/average-rating/:id").get(event.calculateAverageRatingForEvent);
